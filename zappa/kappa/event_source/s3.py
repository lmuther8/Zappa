...
class S3EventSource(kappa.event_source.base.EventSource):
    ...
    def remove(self, function):
        ...
        response = self._s3.call("get_bucket_notification", Bucket=self._get_bucket_name())
        LOG.debug(response)
        if "CloudFunctionConfiguration" in response:
            ...
            if fn_arn == function.arn:
                ...
                response = self._s3.call(
                    "put_bucket_notification", Bucket=self._get_bucket_name(), NotificationConfiguration=response
                )
                LOG.debug(response)
    ...
    def status(self, function):
        ...
        if "CloudFunctionConfiguration" not in response:
            return None
        return {"EventSourceArn": response["CloudFunctionConfiguration"]["CloudFunction"], "State": "Enabled"}
