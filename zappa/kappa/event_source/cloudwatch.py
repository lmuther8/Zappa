...
class CloudWatchEventSource(kappa.event_source.base.EventSource):
    ...
    def add(self, function):
        kwargs = {"Name": self._name, "State": "ENABLED" if self.enabled else "DISABLED"}
        ...
        try:
            ...
            LOG.debug(response)
            response = self._events.call("put_targets", Rule=self._name, Targets=[{"Id": function.name, "Arn": function.arn}])
            LOG.debug(response)
        except Exception:
    ...
    def remove(self, function):
        ...
        try:
            rule = self.get_rule()
            if rule:
                response = self._events.call("remove_targets", Rule=self._name, Ids=[function.name])
                LOG.debug(response)
                response = self._events.call("delete_rule", Name=self._name)
                LOG.debug(response)
        except Exception:
    ...
    def _to_status(self, rule):
        if rule:
            return {"EventSourceArn": rule["Arn"], "State": rule["State"]}
        return None
