...
class KinesisEventSource(kappa.event_source.base.EventSource):
    ...
    def _get_uuid(self, function):
        uuid = None
        response = self._lambda.call("list_event_source_mappings", FunctionName=function.name, EventSourceArn=self.arn)
        LOG.debug(response)
    ...
    def enable(self, function):
        ...
        try:
            response = self._lambda.call("update_event_source_mapping", UUID=self._get_uuid(function), Enabled=self.enabled)
            LOG.debug(response)
        except Exception:
    ...
    def disable(self, function):
        ...
        try:
            response = self._lambda.call("update_event_source_mapping", FunctionName=function.name, Enabled=self.enabled)
            LOG.debug(response)
        except Exception:
    ...
    def update(self, function):
        ...
        if uuid:
            try:
                response = self._lambda.call(
                    "update_event_source_mapping", BatchSize=self.batch_size, Enabled=self.enabled, FunctionName=function.arn
                )
                LOG.debug(response)
            except Exception:
