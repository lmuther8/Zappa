...
class Function(object):
    ...
    excluded_dirs = ["boto3", "botocore", "concurrent", "dateutil", "docutils", "futures", "jmespath", "python_dateutil"]
    excluded_files = [".gitignore"]

    def __init__(self, context, config):
        self._context = context
        self._config = config
        self._lambda_client = kappa.awsclient.create_client("lambda", context.session)
        self._response = None
        self._log = None
    ...
    def deployment_uri(self):
        return "https://{}.execute-api.{}.amazonaws.com/{}".format(
            self.api_id, self._apigateway_client.region_name, self._context.environment
        )

    def _get_response(self):
        if self._response is None:
            try:
                self._response = self._lambda_client.call("get_function", FunctionName=self.name)
    ...
    def _zip_lambda_dir(self, zipfile_name, lambda_dir):
        ...
        with zipfile.ZipFile(zipfile_name, "a", compression=zipfile.ZIP_DEFLATED) as zf:
    ...
    def list_aliases(self):
        ...
        try:
            response = self._lambda_client.call("list_aliases", FunctionName=self.name)
            LOG.debug(response)
        except Exception:
    ...
    def create_alias(self, name, description, version=None):
        ...
        try:
            ...
            response = self._lambda_client.call(
                "create_alias", FunctionName=self.name, Description=description, FunctionVersion=version, Name=name
            )
            LOG.debug(response)
        except Exception:
    ...
    def update_alias(self, name, description, version=None):
        ...
        try:
            ...
            response = self._lambda_client.call(
                "update_alias", FunctionName=self.name, Description=description, FunctionVersion=version, Name=name
            )
            LOG.debug(response)
        except Exception:
            LOG.exception("Unable to update alias")

    def add_permission(self, action, principal, source_arn=None, source_account=None):
        try:
            kwargs = {
            ...
            response = self._lambda_client.call("add_permission", **kwargs)
            LOG.debug(response)
        except Exception:
    ...
    def add_permissions(self):
        ...
        for permission in self.permissions:
            self.add_permission(
                permission["action"], permission["principal"], permission.get("source_arn"), permission.get("source_account")
            )
        ...
        if self._check_function_md5():
            ...
            with open(self.zipfile_name, "rb") as fp:
                try:
                    ...
                    zipdata = fp.read()
                    response = self._lambda_client.call(
                        "update_function_code", FunctionName=self.name, ZipFile=zipdata, Publish=True
                    )
                    LOG.debug(response)
                    self.update_alias(self._context.environment, "For the {} stage".format(self._context.environment))
                except Exception:
    ...
    def list_versions(self):
        try:
            response = self._lambda_client.call("list_versions_by_function", FunctionName=self.name)
            LOG.debug(response)
        except Exception:
    ...
    def delete(self):
        ...
        response = None
        try:
            response = self._lambda_client.call("delete_function", FunctionName=self.name)
            LOG.debug(response)
        except ClientError:
    ...
    def status(self):
        try:
            response = self._lambda_client.call("get_function", FunctionName=self.name)
            LOG.debug(response)
        except ClientError:
            LOG.debug("function %s not found", self.name)
            response = None
    ...
    def _invoke(self, data, invocation_type):
        ...
        response = self._lambda_client.call(
            "invoke", FunctionName=self.name, InvocationType=invocation_type, LogType="Tail", Payload=data
        )
        LOG.debug(response)
        return response
