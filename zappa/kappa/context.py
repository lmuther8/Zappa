...
class Context(object):
    ...
    def __init__(self, config_file, environment=None, debug=False, recording_path=None):
        if debug:
        ...
        if self.environment not in self.config.get("environments", {}):
            message = "Invalid environment {0} specified".format(self.environment)
            LOG.error(message)
            sys.exit(1)
        ...
        if recording_path:
            ...
            self.pill.record()
        self.policy = kappa.policy.Policy(self, self.config["environments"][self.environment])
        self.role = kappa.role.Role(self, self.config["environments"][self.environment])
        self.function = kappa.function.Function(self, self.config["lambda"])
        if "restapi" in self.config:
            self.restapi = kappa.restapi.RestApi(self, self.config["restapi"])
        else:
            self.restapi = None
        self.event_sources = []
    ...
    def unit_test_runner(self):
        return self.config.get("unit_test_runner", "nosetests . ../{}/unit/".format(self.test_dir))
    ...
    def _create_event_sources(self):
        ...
        for event_source_cfg in event_sources:
            ...
            if not event_source:
                raise ValueError("Unknown event source: {0}".format(event_source_cfg["arn"]))
            self.event_sources.append(event_source(self, event_source_cfg))
