...
@click.option("--debug/--no-debug", default=False, help="Turn on debugging output")
@click.option("--env", default="dev", help="Specify which environment to work with (default dev)")
@click.option(
    "--record-path",
...
def status(ctx):
    ...
    if status["function"]:
        line = "    {} ({})".format(
            status["function"]["Configuration"]["FunctionName"], status["function"]["Configuration"]["FunctionArn"]
        )
        click.echo(click.style(line, fg="green"))
    else:
