...
class Policy(object):
    ...
    def __init__(self, context, config):
        ...
        self.config = config
        self._iam_client = kappa.awsclient.create_client("iam", self.context.session)
    ...
    def description(self):
        return "A kappa policy to control access to {} resources".format(self.context.environment)

    def document(self):
        if "resources" not in self.config["policy"] and "statements" not in self.config["policy"]:
            return None
        ...
        for resource in self.config["policy"].get("resources", []):
            ...
            statement = {"Effect": "Allow", "Resource": resource["arn"]}
            actions = []
    ...
    def _find_all_policies(self):
        try:
            response = self._iam_client.call("list_policies", PathPrefix=self._path_prefix)
        except Exception:
    ...
    def _list_versions(self):
        try:
            response = self._iam_client.call("list_policy_versions", PolicyArn=self.arn)
        except Exception:
    ...
    def _add_policy_version(self):
        ...
        if len(versions) == 5:
            try:
                response = self._iam_client.call(
                    "delete_policy_version", PolicyArn=self.arn, VersionId=versions[-1]["VersionId"]
                )
            except Exception:
        ...
        try:
            response = self._iam_client.call(
                "create_policy_version", PolicyArn=self.arn, PolicyDocument=document, SetAsDefault=True
            )
            LOG.debug(response)
        except Exception:
        ...
        if self.arn and document:
            ...
            for version in versions:
                ...
                if not version["IsDefaultVersion"]:
                    try:
                        response = self._iam_client.call(
                            "delete_policy_version", PolicyArn=self.arn, VersionId=version["VersionId"]
                        )
                    except Exception:
                        LOG.exception("Unable to delete policy version %s", version["VersionId"])
            LOG.debug("now delete policy")
            response = self._iam_client.call("delete_policy", PolicyArn=self.arn)
            LOG.debug(response)
        return response
