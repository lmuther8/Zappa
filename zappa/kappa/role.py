...
class Role(object):
    ...
    def __init__(self, context, config, iam_client=None):
        ...
        if not iam_client:
            self._iam_client = kappa.awsclient.create_client("iam", context.session)
        self._arn = None
        ...
        if not role:
            try:
                response = self._iam_client.call(
                    "create_role", Path=self.Path, RoleName=self.name, AssumeRolePolicyDocument=AssumeRolePolicyDocument
                )
                LOG.debug(response)
                if self._context.policy:
                    LOG.debug("attaching policy %s", self._context.policy.arn)
                    response = self._iam_client.call(
                        "attach_role_policy", RoleName=self.name, PolicyArn=self._context.policy.arn
                    )
                    LOG.debug(response)
            except ClientError:
    ...
    def delete(self):
        ...
        try:
            ...
            if policy_arn:
                response = self._iam_client.call("detach_role_policy", RoleName=self.name, PolicyArn=policy_arn)
                LOG.debug(response)
            response = self._iam_client.call("delete_role", RoleName=self.name)
            LOG.debug(response)
        except ClientError:
