...
class Log(object):
    ...
    def __init__(self, context, log_group_name):
        ...
        self._log_client = kappa.awsclient.create_client("logs", context.session)
    ...
    def streams(self):
        ...
        if not self._check_for_log_group():
            LOG.info("log group %s has not been created yet", self.log_group_name)
            return []
        response = self._log_client.call("describe_log_streams", logGroupName=self.log_group_name)
        LOG.debug(response)
    ...
    def tail(self):
        ...
        response = self._log_client.call(
            "get_log_events", logGroupName=self.log_group_name, logStreamName=latest["logStreamName"]
        )
        LOG.debug(response)
    ...
    def delete(self):
        try:
            response = self._log_client.call("delete_log_group", logGroupName=self.log_group_name)
            LOG.debug(response)
        except ClientError:
