...
class RestApi(object):
    ...
    def __init__(self, context, config):
        ...
        self._config = config
        self._apigateway_client = kappa.awsclient.create_client("apigateway", context.session)
        self._api = None
    ...
    def arn(self):
        ...
        arn = "arn:aws:execute-api:{}:{}:{}/*/*/{}".format(region, account, self.api_id, self.resource_name)
        return arn
    ...
    def _get_api(self):
        if self._api is None:
            try:
                response = self._apigateway_client.call("get_rest_apis")
                LOG.debug(response)
    ...
    def create_restapi(self):
        if not self.api_exists():
            ...
            try:
                response = self._apigateway_client.call("create_rest_api", name=self.api_name, description=self.description)
                LOG.debug(response)
            except Exception:
                ...
                if full_path not in resources and parent:
                    try:
                        response = self._apigateway_client.call(
                            "create_resource", restApiId=self.api_id, parentId=parent["id"], pathPart=part
                        )
                        LOG.debug(response)
    ...
    def create_method(self, method, config):
        ...
        try:
            ...
            uri = "arn:aws:apigateway:{}:".format(self._apigateway_client.region_name)
    ...
    def create_deployment(self):
        LOG.info("creating a deployment for %s to stage: %s", self.api_name, self._context.environment)
        try:
            response = self._apigateway_client.call(
                "create_deployment", restApiId=self.api_id, stageName=self._context.environment
            )
            LOG.debug(response)
    ...
    def add_permission(self):
        ...
        self._context.function.add_permission(
            action="lambda:InvokeFunction", principal="apigateway.amazonaws.com", source_arn=self.arn
        )
    ...
    def delete(self):
        ...
        try:
            response = self._apigateway_client.call("delete_resource", restApiId=self.api_id, resourceId=self.resource_id)
            LOG.debug(response)
        except ClientError:
    ...
    def status(self):
        try:
            response = self._apigateway_client.call("delete_", FunctionName=self.name)
            LOG.debug(response)
        except ClientError:
